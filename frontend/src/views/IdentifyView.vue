  ...
  <div class="identify-page">
    ...
    <div v-if="step === 1" class="step-card">
      ...
      <div class="group-selector">
        <div v-for="g in groups" :key="g.value" class="group-option" @click="selectGroup(g.value)">
          <span class="group-option__icon">{{ g.icon }}</span>
          <span class="group-option__label">{{ g.label }}</span>
        </div>
      </div>
    </div>
    ...
    <div v-if="step === 2" class="step-card">
      ...
      <div class="species-suggestions">
        <div v-for="s in suggestions" :key="s.id" class="suggestion-item" @click="selectSpecies(s)">
          <span class="suggestion-icon">{{ s.is_poisonous ? '⚠️' : '✅' }}</span>
          <div class="suggestion-info">
            <strong>{{ s.name_ru }}</strong>
            <span class="latin">{{ s.name_latin }}</span>
          </div>
          <span v-if="s.conservation_status" class="redbook-tag">{{ s.conservation_status }}</span>
        </div>
      </div>
...
const groups = [
  { value: 'plants', icon: '🌿', label: 'Растение' },
  { value: 'fungi', icon: '🍄', label: 'Гриб' },
  { value: 'insects', icon: '🐛', label: 'Насекомое' },
  { value: 'herpetofauna', icon: '🐍', label: 'Герпетофауна' },
  { value: 'birds', icon: '🐦', label: 'Птица' },
  { value: 'mammals', icon: '🦔', label: 'Млекопитающее' },
]
...
.group-selector { display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; }
.group-option { display: flex; flex-direction: column; align-items: center; gap: 8px; padding: 24px 16px; border: 1.5px solid #D6E0E3; border-radius: 16px; cursor: pointer; transition: all 0.2s; }
.group-option:hover { border-color: #2A7A6E; background: rgba(42,122,110,0.04); transform: translateY(-2px); box-shadow: 0 4px 16px rgba(44,62,74,0.1); }
.group-option__icon { font-size: 40px; }
.group-option__label { font-size: 14px; font-weight: 700; color: #2C3E4A; }
.species-suggestions { max-height: 400px; overflow-y: auto; display: flex; flex-direction: column; gap: 6px; }
.suggestion-item { display: flex; align-items: center; gap: 12px; padding: 12px 16px; border-radius: 8px; cursor: pointer; transition: background 0.2s; }
.suggestion-item:hover { background: rgba(42,122,110,0.06); }
.suggestion-icon { font-size: 18px; flex-shrink: 0; }
.suggestion-info { flex: 1; }
.suggestion-info strong { display: block; font-size: 14px; color: #2C3E4A; }
.suggestion-info .latin { font-style: italic; font-size: 12px; color: #8FA5AB; }
.redbook-tag { font-size: 10px; font-weight: 700; color: #E53935; background: rgba(229,57,53,0.1); padding: 2px 8px; border-radius: 4px; white-space: nowrap; }
