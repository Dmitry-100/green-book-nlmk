    ...
    <div class="form-card">
      <div class="form-section">
        ...
        <div class="group-selector">
          <div v-for="g in groups" :key="g.value" class="group-option" :class="{ selected: form.group === g.value }" @click="form.group = g.value">
...
.group-selector { display: grid; grid-template-columns: repeat(6, 1fr); gap: 8px; }
.group-option { display: flex; flex-direction: column; align-items: center; gap: 6px; padding: 14px 8px; border: 1.5px solid #D6E0E3; border-radius: 12px; cursor: pointer; transition: all 0.2s; background: #FAFBFC; }
.group-option:hover { border-color: #2A7A6E; }
.group-option.selected { border-color: #2A7A6E; background: rgba(42,122,110,0.06); box-shadow: 0 0 0 3px rgba(42,122,110,0.1); }
.group-option__icon { font-size: 28px; }
.group-option__label { font-size: 10px; font-weight: 700; text-transform: uppercase; color: #4A6572; text-align: center; }
